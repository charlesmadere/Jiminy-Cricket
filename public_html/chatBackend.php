	...
	if ($user)
	{
		...
		if ($DB_CONNECTION)
		{
			...
			// inserts a new message into the table and also removes any messages past the $STORE_NUM mark
			{
				...
				mysql_query("INSERT INTO $DB_TABLE (`user`, `message`, `time`, `game`) VALUES ('" . $userInfo['name'] . "', '$message', '$epochTime', '$game')", $DB_CONNECTION);
			...
			$chatMessages = mysql_query("SELECT user, message FROM $DB_TABLE WHERE time>$time AND game = '$game'", $DB_CONNECTION);
			...
			// ensure that the table isn't empty and that we can therefore read messages from it
			{
				...
				// loop until there are no more messages in the table left to fetch
				{
					...
					echo "\t\t<text>" . htmlspecialchars(stripslashes($message["message"])) . "</text>\n";
					echo "\t</message>\n";
				}
			}
			echo "</response>\n";
